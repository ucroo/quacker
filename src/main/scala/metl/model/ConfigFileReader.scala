...
trait ConfigFileReader extends Logger {
	...
	protected def getBool(e:Node,tag:String):Option[Boolean] = getText(e,tag) match {
		case Some(t) if t.isInstanceOf[String] && t.toLowerCase.trim == "true" => Option(true)
		case Some(t) if t.isInstanceOf[String] && t.toLowerCase.trim == "false" => Option(false)
		case _ => None
	}
