...
/*
 this is an import of the original net.liftweb comet code, from 3.4.0
 https://github.com/lift/framework/blob/master/web/webkit/src/main/scala/net/liftweb/builtin/snippet/Comet.scala
 but with one line changed (line 70 has been commented out and replaced with line 71).
 The original copyright notice from the original net.liftweb comet code follows:
 */
/*
...
object Comet extends DispatchSnippet with LazyLoggable {
  ...
  private[this] def buildContainer(cometHtml: NodeSeq, cometActor: LiftCometActor, containerId: String): NodeSeq = {
    val cometVersion: String = http.S.requestCometVersions.is
      .collectFirst {
  ...
  /** Given a comet actor and the HTML contents for that actor, renders it into a container and returns the results.
    * Waits up to the timeout specified by the comet actor's `cometRenderTimeout` property and then fails if the actor
    * has not yet properly rendered.
    */
...
//          cometActor.buildSpan(response.inSpan) ++ response.outSpan,
          CometRenderOrder.combineFixedAndDynamic(response.outSpan, cometActor.buildSpan(response.inSpan)),
          cometActor,
  ...
  /** A typical comet tag could look like:
    ...
    *   - You can use a query parameter using `metaname`; e.g., for a url like `foo?id=122`, your comet could take the
    *     name "122" if you use: `metaname=id`
