...
private[http] trait LiftMerge {
  ...
  def merge(xhtml: NodeSeq, req: Req): Node = {
    ...
    def normalizeMergeAndExtractEvents(nodes: NodeSeq, startingState: HtmlState): NodesAndEventJs = {
      ...
      nodes.foldLeft(NodesAndEventJs(Vector[Node](), Noop)) { case (soFar, node) =>
        val childInfo =
          node match {
    ...
    if (!hasHtmlHeadAndBody) {
      ...
      fixedHtml.collectFirst { case e: Elem =>
            ...
            (
              if (autoIncludeComet) {
                ("data-lift-session-id" -> (S.session.map(_.uniqueId) openOr "xx")) ::
                  S.requestCometVersions.is.toList.map { case CometVersionPair(guid, version) =>
        ...
        LiftRules.xhtmlValidator.toList.flatMap(_(tmpRet)) match {
          ...
          case xs =>
            ...
            val errors: NodeSeq = xs.map(e =>
              <div style="border: red solid 2px">XHTML Validation error:{e.msg}at line{e.line + 1}and column{
                e.col
              }</div>
            )
