...
object LiftRulesMocker {
  ...
  /** In Dev and Test mode, there's an option to stuff another LiftRules instance in here and use that one for mocking
    */
  object devTestLiftRulesInstance extends ThreadGlobal[LiftRules]

  protected var overrideLiftRules: Option[LiftRules] = None
  def setOverrideLiftRules(lr: Option[LiftRules]): Unit = {
    overrideLiftRules = lr
  }

  /** This function, in Test and Dev mode will vend the instance of LiftRules. If there is an instance set in
    * devTestLiftRulesInstance, that instance will be used, otherwise the global instance in LiftRules.prodInstance will
    * be used.
    */
  @volatile var calcLiftRulesInstance: () => LiftRules =
    () => overrideLiftRules.getOrElse(devTestLiftRulesInstance.box.openOr(LiftRules.prodInstance))
}

/** The data structure that contains information to determine if the request should be treated as a stateful or
  * stateless request
  */
final case class StatelessReqTest(path: List[String], httpReq: HTTPRequest)

/** Sometimes we're going to have to surface more data from one of these requests than we might like (for example, extra
  * info about continuing the computation on a different thread), so we'll start off right by having an Answer trait
...
object LiftRules extends LiftRulesMocker {
  ...
  private[this] var provider: Option[HTTPProvider] = None
  def setHTTPProvider(prov: Option[HTTPProvider]): Unit = {
    provider = prov
  }
  def getHTTPProvider: Option[HTTPProvider] = {
    provider
  }
  ...
  /** The test between the path of a request, the HTTP request, and whether that path should result in stateless
  ...
  /** A partial function that allows the application to define requests that should be handled by lift rather than the
    * default handler
    */
...
class LiftRules() extends Factory with FormVendor with LazyLoggable {
  ...
  /** If you want to make the Lift inactivity timeout shorter than the container inactivity timeout, set the inactivity
    * timeout here
    */
  ...
  /** Set the default fadeout mechanism for Lift notices. Thus you provide a function that take a NoticeType.Value and
    * decide the duration after which the fade out will start and the actual fadeout time. This is applicable for
  ...
  val noticesAutoFadeOut =
    new FactoryMaker[(NoticeType.Value) => Box[(TimeSpan, TimeSpan)]]((notice: NoticeType.Value) => Empty) {}

  /** Use this to apply various effects to the notices. The user function receives the NoticeType and the id of the
    * element containing the specific notice. Thus it is the function's responsibility to form the javascript code for
    * the visual effects. This is applicable for both ajax and non ajax contexts. For notices associated with ID's the
    * user type will receive an Empty notice type. That's because the effect is applied on the real estate holding the
    * notices for this ID. Typically this contains a single message.
    */
  val noticesEffects =
    new FactoryMaker[(Box[NoticeType.Value], String) => Box[JsCmd]]((notice: Box[NoticeType.Value], id: String) =>
      Empty
    ) {}

  /** Holds user functions that will be executed very early in the request processing. The functions' result will be
    * ignored.
    */
  val early = RulesSeq[(HTTPRequest) => Any]

  /** Holds user functions that are executed before sending the response to client. The functions' result will be
    * ignored.
    */
  ...
  /** The security rules used by Lift to secure this application. These mostly relate to HTTPS handling and HTTP
    * `Content-Security-Policy`. See the `[[SecurityRules]]` documentation for more.
    *
    * Once the application has started using these, they are locked in, so make sure to set them early in the boot
    * process.
    */
  @volatile var securityRules: () => SecurityRules = () => defaultSecurityRules
  private[http] lazy val lockedSecurityRules       = securityRules()

  /** Defines the resources that are protected by authentication and authorization. If this function is not defined for
    * the input data, the resource is considered unprotected ergo no authentication is performed. If this function is
    * defined and returns a Full box, it means that this resource is protected by authentication, and authenticated
    * subjed must be assigned to the role returned by this function or to a role that is child-of this role. If this
    * function returns Empty it means that this resource is protected by authentication but no authorization is
    * performed meaning that roles are not verified.
    */
  ...
  /** A session identifier for [[net.liftweb.http.provider.servlet.HTTPServletSession]].
    *
    * Under most circumstances, you won't need to change this value. However, in some cases containers will be
    * configured with backing datastores that don't play nice with the default value. In those cases you can change this
    * string to get those working.
    */
  @volatile var servletSessionIdentifier: String = "$lift_magic_session_thingy$"

  /** A function that takes the HTTPSession and the contextPath as parameters and returns a LiftSession reference. This
    * can be used in cases subclassing LiftSession is necessary.
    */
  @volatile var sessionCreator: (HTTPSession, String) => LiftSession = { case (httpSession, contextPath) =>
  ...
  /** A method that returns a function to create migratory sessions. If you want migratory sessions for your
    ...
    */
  def sessionCreatorForMigratorySessions: (HTTPSession, String) => LiftSession = { case (httpSession, contextPath) =>
  ...
  /** Attaches an ID entity for resource URI specified in link or script tags. This allows controlling browser resource
    * caching. By default this just adds a query string parameter unique per application lifetime. More complex
    * implementation could user per resource MD5 sequences thus "forcing" browsers to refresh the resource only when the
    * resource file changes. Users can define other rules as well. Inside user's function it is safe to use S context as
  ...
  private def _getLiftSession(req: Req): LiftSession = {
    ...
    val ret = SessionMaster.getSession(req, cometSessionId) match {
      ...
      case _ =>
        ...
        ret.fixSessionTime()
        SessionMaster.addSession(ret, req, req.request.userAgent, SessionMaster.getIpFromReq(req))
        ret
    }
  ...
  /** A function that takes appropriate action in breaking out of any existing comet requests based on the request,
    * browser type, etc.
    */
  ...
  /** Holds the JS library specific UI artifacts. By default it uses JQuery's artifacts
    *
    * Please note that currently any setting other than `JQueryArtifacts` will switch you to using Lift's liftVanilla
    * implementation, which is meant to work independent of any framework. '''This implementation is experimental in
    * Lift 3.0, so use it at your own risk and make sure you test your application!'''
    */
  @volatile var jsArtifacts: JSArtifacts = JQueryArtifacts

  /** Use this PartialFunction to to automatically add static URL parameters to any URL reference from the markup of
    * Ajax request.
    */
  ...
  /** Certain paths and requests within your application can be marked as stateless and if there is access to Lift's
    * stateful facilities (setting SessionVars, updating function tables, etc.) the developer will receive a notice and
  ...
  val statelessSession: FactoryMaker[Req => LiftSession with StatelessSession] =
    new FactoryMaker((req: Req) =>
      new LiftSession(req.contextPath, Helpers.nextFuncName, Empty) with StatelessSession
    ) {}

  /** Holds user functions that are executed after the response is sent to client. The functions' result will be
    * ignored.
    */
  val afterSend = RulesSeq[(BasicResponse, HTTPResponse, List[(String, String)], Box[Req]) => Any]

  /** Calculate the Comet Server (by default, the server that the request was made on, but can do the multi-server thing
    * as well)
    */
  @volatile var cometServer: () => Option[String] = () => None

  /** The maximum concurrent requests. If this number of requests are being serviced for a given session, messages will
    * be sent to all Comet requests to terminate
    */
  ...
  /** When a parameter is received either via POST or GET and does not have a corresponding mapping on the server, the
    * function provided by this FactoryMaker will be called with the req and parameter name.
    *
    * By default, if the parameter looks Lift-like (i.e., it starts with an F), then we log a warning with the given
    * parameter name and URI.
    */
  val handleUnmappedParameter = new FactoryMaker[(Req, String) => Unit](() => { (req: Req, parameterName: String) =>
  ...
  /** Enables or disables event attribute and script element extraction.
    *
    * Lift can extract script elements and event attributes like onclick, onchange, etc, and attach the event handlers
    * in a separate JavaScript file that is generated per-page. This allows for populating these types of JavaScript in
    * your snippets via CSS selector transforms, without needing to allow inline scripts in your content security policy
    * (see `[[securityRules]]`).
    *
    * However, there are certain scenarios where event attribute extraction cannot provide a 1-to-1 reproduction of the
    * behavior you'd get with inline attributes or scripts; if your application hits these scenarios and you would
    * prefer not to adjust them to work with a restrictive content security policy, you can allow inline scripts and set
    * `extractEventAttributes` to false to disable event extraction.
    */
  @volatile var extractInlineJavaScript: Boolean = false

  /** The attribute used to expose the names of event attributes that were removed from a given element for separate
    * processing in JS (when `extractInlineJavaScript` is `true`). By default, Lift removes event attributes and
    * attaches those behaviors via a separate JS file, to avoid inline JS invocations so that a restrictive content
    * security policy can be used.
    *
    * You can set this variable so that the resulting HTML will have attribute information about the removed attributes,
    * in case you need them for e.g. CSS selector matching. The attribute will contain a space-separated list of JS
    * attributes that were removed by Lift's processing.
    *
    ...
    * }}}
    *
    * In Lift 3, this would not work, as the onclick attribute would be removed before HTML serialization, so your HTML
    * would be:
    *
    * {{{
    ...
    * }}}
    *
    * This makes it possible to replace the old CSS with with similar matching for the `data-lift-removed-attributes`
    * attribute:
    *
    * {{{
  ...
  /** Allows user adding additional Lift tags (the tags must be prefixed by lift namespace such as <lift:xxxx/>). Each
    * LiftTagPF function will be called with the following parameters: <pre>
    *   - Element label,
    *   - The Element itselft,
    *   - The attributes
    *   - The child nodes
  ...
  /** If you don't want lift to send the application/xhtml+xml mime type to those browsers that understand it, then set
    * this to { @code false }
    */
  ...
  /** A function that defines how a String should be converted to XML for the localization stuff. By default, Text(s) is
    * returned, but you can change this to attempt to parse the XML in the String and return the NodeSeq.
    */
  ...
  /** This function is called to convert the current set of Notices into a JsCmd that will be executed on the client to
    * display the Notices.
    *
    * @see
  ...
  /** The JsCmd to execute when the comet session is lost. The comet session is considered lost when either (a) a comet
    * request comes in for a session that does not exist on the server or (b) a comet request comes in for a session
    * that has no associated comet actors (this typically happens when the server restarts).
    *
    * By default, we invoke lift.cometOnSessionLost, which can be overridden client-side for more complex work.
  ...
  /** The JsCmd to execute when the ajax session is lost. The ajax session is considered lost when either an ajax
    * request comes in for a session that does not exist on the server.
    *
    * By default, we invoke lift.ajaxOnSessionLost, which can be overridden client-side for more complex work.
  ...
  /** Server-side actors that represent client-side actor endpoints (client actors, Round Trips) need a lifespan. By
    * default, it's 60 seconds, but you might want to make it longer if the client is going to get delayed by long
    * computations that bar it from re-establishing the long polling connection
    */
  val clientActorLifespan = new FactoryMaker[LiftActor => Long](() => (actor: LiftActor) => (30.minutes): Long) {}
  ...
  /** Adjusts LiftRules to allow a page's comets to "rehydrate" (i.e. reconnect to the page) after a server restart.
    * This works by reloading the page into an iframe and stealing all of the comets. When the refresh occurs, new comet
    * actors will be instantiated, so beware that any state therein will NOT be reinstated. Applications utilizing this
    * feature should be designed to reconstruct comet state from the DB, cookies, client info, etc as required.
    */
  ...
  /** Function that generates variants on snippet names to search for, given the name from the template. The default
    * implementation just returns name :: Nil (e.g. no change). The names are searched in order. See also
    * searchSnippetsWithRequestPath for an implementation.
    */
  @volatile var snippetNamesToSearch: FactoryMaker[String => List[String]] =
    new FactoryMaker(() => (name: String) => name :: Nil) {}

  /** Snippet timers are used to time and record the execution time of snippets. We provide two default implementations
    * for you:
    *   - NoOpSnippetTimer that does nothing
    *   - LoggingSnippetTimer that logs snippet times.
    *
    * To enable snippet timing, invoke `LiftRules.installSnippetTimer`. Once enabled you can use the snippet timer like
    * a regular `FactoryMaker`. If you only want snippet timing for certain sessions or requests, invoke
    * `installSnippetTimer` with the `NoOpSnippetTimer` and change the value for the sessions or requests where you want
    * snippet timing.
    *
    * Since this is a `FactoryMaker` you can programmatically override it for an individual request or session as you
    * see fit. You can also implement your own timer!
    */
  ...
  /** Enable snippet timing and install a default snippet timer. This method can only be invoked during boot.
    *
    * This method only enables snippet timing and sets the default snippet timer. If you want to change snipping timing
    * behavior for specific sessions or requests, you'll want to interact with the underlying `FactoryMaker` after its
    * set up.
    */
  ...
  /** Implementation for snippetNamesToSearch that looks first in a package named by taking the current template path.
    * For example, suppose the following is configured in Boot: LiftRules.snippetNamesToSearch.default.set(() =>
    * LiftRules.searchSnippetsWithRequestPath) LiftRules.addToPackages("com.mycompany.myapp")
    * LiftRules.addToPackages("com.mycompany.mylib") The tag <lift:MySnippet> in template foo/bar/baz.html would search
    * for the snippet in the following locations:
  ...
  /** If the request times out (or returns a non-Response) you can intercept the response here and create your own
    * response
    */
  ...
  /** The JavaScript to execute to log a message on the client side when lift.logError is called.
    *
    * If Empty no logging is performed The default when running in DevMode is to call lift.logError which will use
    * JavaScript console if available or alert otherwise.
    *
  ...
  /** An XML header is inserted at the very beginning of returned XHTML pages. This function defines the cases in which
    * such a header is inserted. The function takes a NodeResponse (the LiftResponse that is converting the XML to a
    * stream of bytes), the Node (root node of the XML), and a Box containing the content type.
    */
  ...
  /** Given the current location (based on the Req.path.partPath), what are the resource bundles in the templates for
    * the current page.
    *
    * @see
  ...
  /** Ever wanted to add custom attribute processing to Lift? Here's your chance. Every attribute with the data- prefix
    * will be tested against this RulesSeq and if there's a match, then use the rule process. Simple, easy, cool.
    */
  val dataAttributeProcessor: RulesSeq[DataAttributeProcessor] = new RulesSeq()

  dataAttributeProcessor.append { case ("lift", snippetInvocation, element, liftSession) =>
      ...
      case snippetName :: encodedArguments =>
        ...
        if (decodedMetaData.get("parallel").headOption == Some(Text("true"))) {
          DataAttributeProcessorAnswerFuture(
            LAFuture(() => new Elem("lift", snippetName, decodedMetaData, element.scope, false, element))
          )
        } else {
  ...
  /** Ever wanted to match on *any* arbitrary tag in your HTML and process it any way you wanted? Well, here's your
    * chance, dude. You can capture any tag and do anything you want with it.
    *
    * Note that this set of PartialFunctions is run for **EVERY** node in the DOM so make sure it runs *FAST*.
    *
    * Also, no subsequent processing of the returned NodeSeq is done (no LiftSession.processSurroundAndInclude()) so
    * evaluate everything you want to.
    *
    * But do avoid infinite loops, so make sure the PartialFunction actually returns true *only* when you're going to
  ...
  /** There may be times when you want to entirely control the templating process. You can insert a function to this
    * factory that will do your custom template resolution. If the PartialFunction isDefinedAt the given locale/path,
    * then that's the template returned. In this way, you can return Empty for a template that's not found and the
    * template will not be found. Otherwise, if the function is not defined for the locale/path pair, the normal
    * templating system will be used. Also, keep in mind how FactoryMaker can be used... it can be global, per request,
    * etc.
    */
  ...
  /** There may be times when you want to entirely control the templating process. You can insert a function that
    * creates a white list of snippets. The white list is the exhaustive list of snippets. The snippets are class/method
    * pairs. If the partial function is defined and the result is a Full Box, the function is run. If the Box is an
    * EmptyBox, then the result is a snippet lookup failure. If the partial function is not defined, then the normal
    * snippet resolution mechanism is used. Please note that in Scala a Map is PartialFunction and you can create Maps
    * that have a default value using the withDefaultValue method.
    */
  ...
  /** Set the sitemap to a function that will be run to generate the sitemap.
    *
    * This allows for changing the SiteMap when in development mode and having the function re-run for each
    * request.<br/>
    *
    * This is **NOT** a mechanism for dynamic SiteMap. This is a mechanism **ONLY** for allowing you to change the
    * SiteMap during development. There will be significant performance penalties (serializing the service of
    * requests... only one at a time) for changing the SiteMap.
    */
  ...
  /** Return the sitemap if set in Boot. If the current runMode is development mode, the sitemap may be recomputed on
    * each page load.
    */
  ...
  /** A unified set of properties for managing how to treat HTML, XHTML, HTML5. The default behavior is to return an
  ...
  val deferredSnippetFailure: FactoryMaker[Failure => NodeSeq] =
    new FactoryMaker(() => { failure: Failure =>
      {
        if (Props.devMode)
          <div style="border: red solid 2px">A lift:parallel snippet failed to render.Message:{failure.msg}{
  ...
  /** Holds user's DispatchPF functions that will be executed in a stateless context. This means that no session will be
    * created and no JSESSIONID cookie will be presented to the user (unless the user has presented a JSESSIONID
    * cookie).
    *
  ...
  /** Add functionality around all of the HTTP request/response cycle. This is an optimal place to get a database
    * connection. Note that whatever is loaned at the beginning of the request will not be returned until the end of the
    * request. It's super-important to (1) not do anything related to state or touch the request objects or anything
    * else at the beginning or end of the loan wrapper phase; (2) make sure that your code does not throw exceptions as
    * exceptions can cause major problems.
    */
  ...
  /** The root path for the assets folder.
    *
    * This applies also to the URL domain, appended after the deployed context path. In app developer's work area this
    * folder resides under the 'webapp' folder.
    *
    * For example to store asset files under 'myassets' folder, use value '/myassets/'. Assets under this configurable
    * path follow conventions, for example the default lazy loading spinner ('ajax-loader.gif') is under 'images'
    * folder.
    *
  ...
  /** Contains the URI path under which all built-in Lift-handled requests are scoped. It does not include the context
    * path and should not begin with a /.
    */
  ...
  /** If there is an alternative way of calculating the context path (by default returning Empty)
    ...
    */
  @volatile var calculateContextPath: Option[net.liftweb.http.provider.HTTPRequest] => Box[String] =
    (o: Option[net.liftweb.http.provider.HTTPRequest]) => Empty
  ...
  /** In Dev mode and Test mode, return a non-200 response code if there is an error on the page (one that would result
    * in the red box with the error message being displayed). This helps in testing automation.
    */
  ...
  /** Obtain the resource as an XML by name. If you're using this to load a template, consider using the Template object
    * instead.
    *
    * @see
    *   Template
  ...
  /** Get the partial function that defines if a request should be handled by the application (rather than the default
    * container handler)
    */
  ...
  /** Holds the user's rewrite functions that can alter the URI parts and query parameters. This rewrite is performed
    * very early in the HTTP request cycle and may not include any state. This rewrite is meant to rewrite requests for
    * statelessDispatch. <br/> Note also that rewrites should not have side effects except to memoize database query
    * results. No side effects means that you should not change SessionVars in a rewrite.
    */
  val statelessRewrite = RulesSeq[RewritePF]

  /** Holds the user's rewrite functions that can alter the URI parts and query parameters. This rewrite takes place
    * within the scope of the S state so SessionVars and other session-related information is available. <br/> Note also
    * that rewrites should not have side effects except to memoize database query results. No side effects means that
    * you should not change SessionVars in a rewrite. <br/> In general, rewrites should be considered low level access.
    * Rather than using a rewrite to extract parameters out of a URL, you'll be much better off using SiteMap generally
  ...
  /** Handles the parsing of template content into NodeSeqs. If multiple parsers are registered for the same template
    * suffix, the first matching parser is used. This intended to be set in in `Boot` as it is read only once during the
    * processing of the first template.
    */
  ...
  /** Execute certain functions early in a Stateful Request This is called early in a stateful request (one that's not
    * serviced by a stateless REST request and one that's not marked as a stateless HTML page request).
    *
    * DPP strongly recommends that everything that you do related to user state is done with `earlyInStateful`, instead
    * of using `[[onBeginServicing]]`.
    */
  ...
  /** Sometimes the comet logger (which is really the Ajax logger) needs to have the string cleaned up to remove stuff
    * like passwords. That's done by this function.
    */
  ...
  @volatile var defaultHeaders: PartialFunction[(NodeSeq, Req), List[(String, String)]] = { case _ =>
  ...
  /** convertResponse is a PartialFunction that reduces a given Tuple4 into a LiftResponse that can then be sent to the
    * browser.
    */
  ...
  val guardedSettingViolationFunc = new LiftRulesGuardedSetting[LiftRulesGuardedSetting.SettingViolation => Unit](
  ...
  /** Set to false if you do not want ajax/comet requests that are not associated with a session to call their
    * respective session loss handlers (set via LiftRules.noAjaxSessionCmd and LiftRules.noCometSessionCmd).
    */
  @volatile var redirectAsyncOnSessionLoss = true

  /** The sequence of partial functions (pattern matching) for handling converting an exception to something to be sent
    * to the browser depending on the current RunMode (development, etc.)
    *
    * By default it returns an XhtmlResponse containing a predefined markup. You can overwrite this by calling
    * LiftRules.exceptionHandler.prepend(...). If you are calling append then your code will not be called since a
    * default implementation is already appended.
    */
  ...
  /** The list of partial function for defining the behavior of what happens when URI is invalid and you're not using a
    * site map
    */
  val uriNotFound = RulesSeq[URINotFoundPF].prepend(NamedPF("default") { case (r, _) =>
    DefaultNotFound
  })

  /** If you use the form attribute in a snippet invocation, what attributes should be copied from the snippet
    * invocation tag to the form tag. The default list is "class", "id", "target", "style", "onsubmit"
    */
  val formAttrs: FactoryMaker[List[String]] = new FactoryMaker(() =>
    List("class", "id", "target", "style", "onsubmit")
  ) {}

  /** By default, Http response headers are appended. However, there are some headers that should only appear once (for
    * example "expires"). This Vendor vends the list of header responses that can only appear once.
    */
  ...
  /** Modifies the root relative paths from the css url-s
    ...
    * @param prefix
    *   the prefix to be added on the root relative paths. If this is Empty, the prefix will be the application context
    *   path.
    */
  def fixCSS(path: List[String], prefix: Box[String]) {
    ...
    val cssFixer: LiftRules.DispatchPF = new LiftRules.DispatchPF {
      ...
      def apply(r: Req): () => Box[LiftResponse] = {
        ...
        () => {
          css.map(str =>
            CSSHelpers.fixCSS(new BufferedReader(new StringReader(str)), prefix openOr (S.contextPath)) match {
  ...
  /** Holds user function hooks when the request is about to be processed It's legacy from when Lift was a lot more
    * Rails-like. It's called literally at the very beginning of the servicing of the HTTP request. The S scope is not
    * available nor is the DB connection available in onBeginServicing. We recommend using earlyInStateful.
    */
  ...
  /** Compute the headers to be sent to the browser in addition to anything else that's sent.
    *
    * Note that the headers for the applications `SecurityRules` are also set here, so if you override the supplemental
    * headers, you should either refer back to the default set or make sure to include
  ...
  /** Handles content security policy violation reports reported to the default reporting endpoint (see
    ...
    *
    * If an `Empty` is returned from this function, a default 200 response will be returned. The default implementation
    * simply logs the violation at WARN level.
    */
  @volatile var contentSecurityPolicyViolationReport: (ContentSecurityPolicyViolation) => Box[LiftResponse] = {
    violation =>
      logger.warn(
  ...
  /** By default lift uses a garbage-collection mechanism of removing unused bound functions from LiftSesssion. Setting
    * this to false will disable this mechanisms and there will be no Ajax polling requests attempted.
    */
  @volatile var enableLiftGC = true;

  /** If Lift garbage collection is enabled, functions that are not seen in the page for this period of time (given in
    * milliseconds) will be discarded, hence eligible for garbage collection. The default value is 10 minutes.
    */
  @volatile var unusedFunctionsLifeTime: Long = 10.minutes

  /** The polling interval for background Ajax requests to prevent functions of being garbage collected. Default value
  ...
  /** The polling interval for background Ajax requests to keep functions to not be garbage collected. This will be
    * applied if the Ajax request will fail. Default value is set to 15 seconds.
    */
  @volatile var liftGCFailureRetryTimeout: Long = 15.seconds

  /** If this is Full, comet updates (partialUpdates or reRenders) are wrapped in a try/catch statement. The provided
    * JsCmd is the body of the catch statement. Within that JsCmd, the variable "e" refers to the caught exception.
    *
    * In development mode, this defaults to Full and the command within invokes lift.cometOnError with the exception;
    * lift.cometOnError rethrows the exception by default. In production mode, this defaults to Empty.
    *
    * Note that if you set this to Full, it is highly advised that you rethrow the exception. If you fail to rethrow the
    * exception, you run the risk of dropping an unpredictable number of updates (i.e., if the third of 20 updates that
    * are sent to the client in a single response throws an exception, none of the subsequent ones will run; failing to
    * rethrow the exception means any updates that did not run will never be run).
    */
  ...
  /** Holds the last update time of the Ajax request. Based on this server may return HTTP 304 status indicating the
    * client to used the cached information.
    */
  ...
  /** When a request is parsed into a Req object, certain suffixes are explicitly split from the last part of the
    * request URI. If the suffix is contained in this list, it is explicitly split. The default list is: "html", "htm",
    * "jpg", "png", "gif", "xml", "rss", "json" ...
    */
  ...
  /** The global multipart progress listener: pBytesRead - The total number of bytes, which have been read so far.
    * pContentLength - The total number of bytes, which are being read. May be -1, if this number is unknown. pItems -
  ...
  /** The function that converts a fieldName, contentType, fileName and an InputStream into a FileParamHolder. By
    * default, create an in-memory instance. Use OnDiskFileParamHolder to create an on-disk version
    */
  ...
  /** This variable controls whether RequestVars that have been set but not subsequently read will be logged in Dev
    * mode. Logging can be disabled at the per-RequestVar level via RequestVar.logUnreadVal
    *
    * @see
  ...
  /** Controls whether or not the service handling timing messages (Service request (GET) ... took ... Milliseconds) are
    * logged. If set to false NoOpServiceTimer is used. We should remove this setting in Lift-4 and only depend on
    * serviceRequestTimer Defaults to true.
    */
  ...
  /** Handles logging of servicing a request two default implementations:
    ...
    *   - StandardServiceTimer that logs time it takes to serve the request (Service request (GET) ... took ...
    *     Milliseconds). This is the default used.
    *
  ...
  private def ctor() {
    ...
    appendGlobalFormBuilder(
      FormBuilderLocator[Int]((value, setter) => SHtml.text(value.toString, s => Helpers.asInt(s).foreach((setter))))
    )
  ...
  class RulesSeq[T] {
    ...
    /** Sometimes it's useful to change the rule for the duration of a thread... prepend a rule and execute the code
      * within a scope with the prepended rule
      */
    def prependWith[A](what: T)(f: => A): A = prependWith(List(what))(f)

    /** Sometimes it's useful to change the rule for the duration of a thread... append a rule and execute the code
      * within a scope with the appended rule
      */
    def appendWith[A](what: T)(f: => A): A = appendWith(List(what))(f)

    /** Sometimes it's useful to change the rule for the duration of a thread... prepend rules and execute the code
      * within a scope with the prepended rules
      */
    ...
    /** Sometimes it's useful to change the rules for the duration of a thread... append rules and execute the code
      * within a scope with the appended rules
      */
