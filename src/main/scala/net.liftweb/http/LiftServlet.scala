...
/** Wrap a LiftResponse and cache the result to avoid computing the actual response more than once
  */
...
class LiftServlet extends Loggable {
  ...
  private def handleGenericContinuation(
      ...
      func: ((=> LiftResponse) => Unit) => Unit
  ): Boolean = {

    val req = if (null eq reqOrg) reqOrg else reqOrg.snapshot

    val timeout = req.getTimeout.getOrElse(cometTimeout)

    def runFunction(doAnswer: LiftResponse => Unit) {
      Schedule.schedule(
        () => {
          val answerFunc: (=> LiftResponse) => Unit = response => doAnswer(wrapState(req, session)(response))

          func(answerFunc)

        },
        TimeSpan(0) // changed from 5ms delay?
      )
    ...
    if (reqOrg.request.suspendResumeSupport_?) {
      try {
        reqOrg.request.suspend(timeout)
      } finally {
        ...
        try {
          S.init(Box !! r2, s2) {
            dispatchStatefulRequest(
              S.request.openOrThrowException("I'm pretty sure this is a full box here"),
              liftSession,
              r2,
              continue
            )
  ...
  /** This is the processing pipeline for all lift requests. Basically each of these takes a Req and returns either a
    * Full(Response) - in which case return Empty - Go to the next handler Failure - short circuit and return
    */
  ...
  private def dispatchStatefulRequest(
      req: Req,
      liftSession: LiftSession,
      originalRequest: Req,
      continuation: Box[() => Nothing]
  ): () => Box[LiftResponse] = {
    val toMatch = req
                ...
                S.functionLifespan(true) {
                  pf(toMatch)() match {
                    case Full(v) =>
                      (
                        true,
                        Full(LiftRules.convertResponse((liftSession.checkRedirect(v), Nil, S.responseCookies, req)))
                      )
  ...
  /** Tracks the two aspects of an AJAX version: the sequence number, whose sole purpose is to identify requests that
    * are retries for the same resource, and pending requests, which indicates how many requests are still queued for
    * this particular page version on the client. The latter is used to expire result data for sequence numbers that are
    * no longer needed.
    */
  ...
  /** Extracts two versions from a given AJAX path:
    ...
    *   - The requestVersions, which let us determine if this is a request we've already dealt with or are currently
    *     dealing with (so we don't rerun the associated handler). See handleVersionedAjax for more.
    *
  ...
  /** Runs the actual AJAX processing. This includes handling __lift__GC, or running the parameters in the session. It
    * returns once the AJAX request has completed with a response meant for the user. In cases where the request is
    * taking to respond, an LAFuture may be wrapped around the execution; see `handleAjax` for more.
    */
  private def runAjax(liftSession: LiftSession, requestState: Req): Box[LiftResponse] = {
    try {
  ...
  /** Kick off AJAX handling. Extracts relevant versions and handles the begin/end servicing requests. Then checks
    * whether to wait on an existing request for this same version to complete or whether to do the actual processing.
    */
  private def handleAjax(liftSession: LiftSession, requestState: Req): Box[LiftResponse] = {
        ...
        request.request.resume(
          (
            ...
            S.init(Box !! request, session)(
              LiftRules.performTransform(convertAnswersToCometResponse(session, answers.toList, actors))
            )
          )
        )
    )
  ...
  private def handleComet(
    ...
    if (actors.isEmpty)
      Left(
        Full(
          new JsCommands(LiftRules.noCometSessionCmd.vend :: js.JE.JsRaw("lift.setToWatch({});").cmd :: Nil).toResponse
        )
      )
    else
  ...
  private def convertAnswersToCometResponse(
      session: LiftSession,
  ...
  private def handleNonContinuationComet(
    ...
    val cont = new ContinuationActor(request, session, actors, answers => f.satisfy(answers))
  ...
  /** Sends the { @code HTTPResponse } to the browser using data from the { @link Response } and { @link Req }.
    */
        ...
        case ("Location", uri) =>
          ...
          (
            ...
            (
              (for (
                updated <- Full(
                  (if (!LiftRules.excludePathFromContextPathRewriting.vend(uri)) u.contextPath else "") + uri
