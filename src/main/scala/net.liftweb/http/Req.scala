...
object UserAgentCalculator extends Factory {
  ...
  /** You can change the mechanism by which the user agent for IE is calculated by doing the Factory thing with this
    * object
    */
  object ieCalcFunction extends FactoryMaker[Box[String] => Box[Double]](defaultIeCalcFunction _)
  ...
  /** You can change the mechanism by which the user agent for Safari is calculated by doing the Factory thing with this
    * object
    */
  object safariCalcFunction extends FactoryMaker[Box[String] => Box[Double]](defaultSafariCalcFunction _)
  ...
  /** You can change the mechanism by which the user agent for Firefox is calculated by doing the Factory thing with
    * this object
    */
  object firefoxCalcFunction extends FactoryMaker[Box[String] => Box[Double]](defaultFirefoxCalcFunction _)
  ...
  /** You can change the mechanism by which the user agent for Chrome is calculated by doing the Factory thing with this
    * object
    */
  object chromeCalcFunction extends FactoryMaker[Box[String] => Box[Double]](defaultChromeCalcFunction _)

  /** You can change the mechanism by which Lift calculates if the User-Agent represents an iPhone. Put your special
    * calculation function in here
    */
  object iPhoneCalcFunction extends FactoryMaker[Box[Box[String] => Boolean]](Empty)

  /** You can change the mechanism by which Lift calculates if the User-Agent represents an iPad. Put your special
    * calculation function in here
    */
  object iPadCalcFunction extends FactoryMaker[Box[Box[String] => Boolean]](Empty)
...
trait UserAgentCalculator {
  ...
  lazy val isIPhone: Boolean =
    UserAgentCalculator.iPhoneCalcFunction.vend.map(_.apply(userAgent)) openOr
      isSafari && (userAgent.map(s => s.indexOf("(iPhone") >= 0) openOr false)
  ...
  lazy val isIPad: Boolean =
    UserAgentCalculator.iPadCalcFunction.vend.map(_.apply(userAgent)) openOr
      isSafari && (userAgent.map(s => s.indexOf("(iPad") >= 0) openOr false)
...
/** A FileParamHolder contains a file uploaded via a multipart form.
  ...
  */
abstract class FileParamHolder(val name: String, val mimeType: String, val fileName: String)
    extends ParamHolder
    with Serializable {
...
/** This FileParamHolder stores the uploaded file directly into memory.
  ...
  *   The local filename on the client
  * @param file
  *   The contents of the uploaded file in a byte array
  */
class InMemFileParamHolder(
...
object Req {
  ...
  def apply(
    ...
    val contextPath = LiftRules.calculateContextPath(Some(request)) openOr request.contextPath
    ...
    val localParams: Map[String, List[String]] = Map(rewritten.params.toList.map { case (name, value) =>
      name -> List(value)
    }: _*)
    ...
    lazy val queryStringParam: (List[String], Map[String, List[String]]) = {
      ...
      val nvp: Map[String, List[String]] = params.foldLeft(Map[String, List[String]]()) { case (map, (name, value)) =>
    ...
    val paramCalcInfo: AvoidGAL = new AvoidGAL(() => {
      ...
      if (
        ...
        val params = normal.foldLeft(eMap)((a, b) => a + (b.name -> (a.getOrElse(b.name, Nil) ::: List(b.value))))
        ...
        val params = localParams ++ (request.params
          .sortWith { (s1, s2) =>
            s1.name < s2.name
          })
  ...
  def parsePath(in: String): ParsePath = {
    ...
    val orgLst = p1.replaceAll("/$", "/index").split("/").toList.map(_.trim).filter(_.length > 0)
...
final case class ParamCalcInfo(
...
final case class ContentType(
  ...
  /** Compares this to another ContentType instance based on the q and if the q matches, compare based on specialization
    * (* vs. explicit and then order.
    */
...
/** The ContentType companion object that has helper methods for parsing Accept headers and other things that contain
  * multiple ContentType information.
  */
object ContentType {

  /** Parse the String into a series of ContentType instances, returning the multiple ContentType instances
    */
...
class Req(
  ...
  protected var timeout: Option[Long] = None
  def overrideTimeout(l: Option[Long]): Unit = {
    timeout = l
  }
  def getTimeout: Option[Long] = timeout
  ...
  /** The POST or PUT body. This will be empty if the content type is application/x-www-form-urlencoded or a multipart
    * mime. It will also be empty if rawInputStream is accessed
    */
  def body: Box[Array[Byte]] = _body.flatMap(_.body)

  /** The raw input stream of a POST or PUT that is not application/x-www-form-urlencoded or a multipart mime and if
    * this method is called before the body method. Remember to close the stream when done.
    */
  ...
  lazy val params: Map[String, List[String]] = addlParams.foldLeft(_params) { case (map, (key, value)) =>
  ...
  /** The JValue representation of this Req's body, if the body is JSON-parsable AND the content-type of the request is
    * JSON. Returns a Failure if the request is not considered a JSON request (see json_?), or if there was an error
    * parsing the JSON.
    *
  ...
  /** Forcibly tries to parse the request body as JSON. Does not perform any content type checks, unlike the json
    * method.
    */
  lazy val forcedBodyAsJson: Box[JsonAST.JValue] = {
    try {
      ...
      body.map(b => JsonParser.parse(new InputStreamReader(new ByteArrayInputStream(b), charSet)))
    } catch {
  ...
  /** The hostname to which the request was sent. This is taken from the "Host" HTTP header, or if that does not exist,
  ...
  /** The host and path of the request up to and including the context path. This does not include the template path or
    * query string.
    */
  ...
  /** The Elem representation of this Req's body, if the body is XML-parsable AND the content-type of the request is
    * XML. Returns a Failure if the request is not considered a XML request (see xml_?), or if there was an error
    * parsing the XML.
    *
  ...
  lazy val accepts: Box[String] = {
    request.headers.toList.filter(_.name equalsIgnoreCase "accept").flatMap(_.values) match {
      case Nil => Empty
  ...
  lazy val acceptsJavaScript_? =
    (weightedAccept.find(_.matches("text" -> "javascript")) orElse
      weightedAccept.find(_.matches("application" -> "javascript"))).isDefined
...
/** Maintains the context of resolving the URL when cookies are disabled from container. It maintains low coupling such
  * as code within request processing is not aware of the actual response that encodes the URL.
  */
object RewriteResponse {
  ...
  def apply(path: List[String], stopRewriting: Boolean) =
    new RewriteResponse(ParsePath(path, "", true, false), Map.empty, stopRewriting)
...
/** Provides access to a thread-local URL rewriter. Typically uses either an applicable entry in
  * `[[LiftRules.urlDecorate]]` or the container's built-in URL decoration which may append the session id to the URL
  * (dependent on `[[LiftRules.encodeJSessionIdInUrl_?]]`).
  */
object URLRewriter {
