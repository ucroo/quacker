...
object LiftSession {
  ...
  /** Holds user's functions that will be called when a function owner is no longer has functions registered for
    * callbacks.
    *
    * Note that owners are '''typically''', though not always, the [[S.renderVersion render version]] of a page, so this
    * is a good way to find out when a page no longer has any client-side functions (e.g., form fields) bound on the
    * server.
    */
  ...
  /** After the session is created, if you need to do anything within the context of the session (like set SessionVars,
    * etc), add the function to this list
    */
...
trait HowStateful {
  ...
  /** There may be cases when you are allowed container state (e.g., migratory session, but you're not allowed to write
    * Lift non-migratory state, return true here.
    */
  def allowContainerState_? = howStateful.box openOr true

  /** Within the scope of the call, this session is forced into statelessness. This allows for certain URLs in on the
    * site to be stateless and not generate a session, but if a valid session is presented, they have the scope of that
    * session/User
    */
...
/** Sessions that include this trait will not be retained past the current request and will give notifications of
  * failure if stateful features of Lift are accessed
  */
...
/** Sessions that include this trait will only have access to the container's state via ContainerVars. This mode is
  * "migratory" so that a session can migrate across app servers. In this mode, functions that access Lift state will
  * give notifications of failure if stateful features of Lift are accessed
  */
...
/** Keeps information around about what kinds of functions are run at the end of page rendering. The results of these
  * functions will be appended to the bottom of the page.
  *
  * @param renderVersion
  * -- the page ID (aka the RenderVersion)
  * @param functionCount
  * -- the number of functions in the collection
  * @param lastSeen
  * -- page of the page-level GC
  * @param functions
  * -- the list of functions to run
  */
private final case class PostPageFunctions(
  ...
  /** Create a new instance based on the last seen time
    */
  def updateLastSeen = new PostPageFunctions(renderVersion, functionCount, longLife, Helpers.millis, functions)

}

/** The responseFuture will be satisfied by the original request handling thread when the response has been calculated.
  * Retries will wait for the future to be satisfied in order to return the proper response.
  */
private[http] final case class AjaxRequestInfo(
    requestVersion: Long,
...
/** The LiftSession class containing the session state information
  */
class LiftSession(private[http] val _contextPath: String, val underlyingId: String, val httpSession: Box[HTTPSession])
    extends LiftMerge
    with Loggable
    with HowStateful {
  ...
  /** The unique id of this session. Can be used to securely and uniquely identify the session, unlike underlyingId
    * which is tied to the host session id. This id should be secure to emit into a page as needed (for example, it is
    * used to provide randomness/cache-busting to the comet request path).
    */
  ...
  /** Was this session marked for shutdown... if so, don't remark
    */
  ...
  /** A mapping between pages denoted by RenderVersion and functions to execute at the end of the page rendering
    */
  @volatile private var postPageFunctions: Map[String, PostPageFunctions] = Map()

  /** A list of AJAX requests that may or may not be pending for this session. There is an entry for every AJAX request
    * we don't *know* has completed successfully or been discarded by the client.
    *
    * See LiftServlet.handleAjax for how we determine we no longer need to hold a reference to an AJAX request.
    */
  ...
  private[http] def startSession(): Unit = {
    ...
    for (sess <- httpSession) {
      ...
      // why bother.
      inactivityLength = (sess.maxInactiveInterval * 1000L, LiftRules.sessionInactivityTimeout.vend) match {
  ...
  def cometForHost(hostAndPath: String): (Vector[(LiftActor, Req)], Vector[(LiftActor, Req)]) =
    asyncSync
      ...
      .foldLeft((Vector[(LiftActor, Req)](), Vector[(LiftActor, Req)]())) { (soFar, current) =>
        (soFar, current) match {
  ...
  def runParams(state: Req): List[Any] = {
    ...
    def buildFunc(i: RunnerHolder): () => Any = i.func match {
      ...
      case normal =>
        () => normal(state.params.getOrElse(i.name, state.uploadedFiles.filter(_.name == i.name).map(_.fileName)))
    }
  ...
  def updateFunctionMap(funcs: Map[String, S.AFuncHolder], uniqueId: String, when: Long): Unit = {
    funcs.foreach { case (name, func) =>
      nmessageCallback.put(name, if (func.owner == Full(uniqueId)) func else func.duplicate(uniqueId))
    }
  }

  /** Removes the function with the given `name`. Note that this will '''not''' trigger
    * `[[LiftSession$.onFunctionOwnersRemoved]]` listeners.
    */
  def removeFunction(name: String) = {
    nmessageCallback.remove(name)
  }

  /** Given a test that takes an [[S.AFuncHolder]] and produces a boolean, remove any bound function in the session that
    * passes the test.
    *
    * If any function owner is completely absent after going through the functions, any function registered via
    * `[[LiftSession$.onFunctionOwnersRemoved]]` will be called with the list of removed owners.
    */
  ...
  /** Set your session-specific progress listener for mime uploads pBytesRead - The total number of bytes, which have
    * been read so far. pContentLength - The total number of bytes, which are being read. May be -1, if this number is
    * unknown. pItems - The number of the field, which is currently being read. (0 = no item so far, 1 = first item is
    * being read, ...)
    */
  ...
  /** Clear the PostPage JavaScript functions for the current page. This is used by CometActor to remove the PostPage
    * JavaScript functions from the given component during redraw.
    */
  ...
  /** Associate a function that renders JavaScript with the current page. This function will be run and the resulting
    * JavaScript will be appended to any rendering associated with this page... the normal page render, Ajax calls, and
    * even Comet calls for this page.
    *
    * @param func
  ...
  def addPostPageJavaScript(func: () => JsCmd) {
    testStatefulFeature {
      accessPostPageFuncs {
        ...
        val old =
          postPageFunctions.getOrElse(rv, PostPageFunctions(rv, 0, S.currentCometActor.isDefined, Helpers.millis, Nil))
  ...
  /** Get the post-page JavaScript functions for a sequence of page IDs. This is used by the CometActor to get the
    * post-page JavaScript functions for the comet actor and for the page the the comet actor is associated with
    */
  ...
  /** Updates the timestamp of the functions owned by this owner and return the number of updated functions
    */
  private[http] def updateFuncByOwner(ownerName: String, time: Long): Int = {
    ...
    import scala.collection.JavaConverters._
    (0 /: nmessageCallback.asScala)((l, v) =>
      l + (v._2.owner match {
  ...
  private def shutDown() = {
    ...
    S.initIfUninitted(this) {
      ...
      this.synchronized {
        ...
        import scala.collection.JavaConverters._
        nasyncComponents.asScala.foreach { case (_, comp) =>
  ...
  /** Define the context path for this session. This allows different sessions to have different context paths.
    */
  def contextPath = LiftRules.calculateContextPath(None) openOr _contextPath
  ...
  /** If the sitemap entry for this Req is marked stateless, run the rest of the request as stateless
    */
  ...
  /** Destroy the current session, then create a new session and continue the execution of the code. The continuation
    * function must return Nothing (it must throw an exception... this is typically done by calling S.redirectTo(...)).
    * This method is useful for changing sessions on login. Issue #727.
    */
  def destroySessionAndContinueInNewSession(continuation: () => Nothing): Nothing = {
    throw new ContinueResponseException(continuation)
  }

  private[http] def processRequest(request: Req, continuation: Box[() => Nothing]): Box[LiftResponse] = {
  ...
  /** Gets the named variable if it exists
    *
    * @param name
    * -- the name of the session-local variable to get
    *
    * @return
    *   Full ( value ) if found, Empty otherwise
    */
  ...
  def runSourceContext(value: Any, xform: NodeSeq => NodeSeq, ns: NodeSeq): NodeSeq = {
    ...
    value match {
      ...
      case n: java.util.Iterator[_] =>
        for {
          i <- n.asScala.toSeq; nodes <- currentSourceContext.doWith(i)(processSurroundAndInclude("Source", xform(ns)))
        } yield nodes
      case en: java.util.Enumeration[_] =>
        for {
          i <- en.asScala.toSeq; nodes <- currentSourceContext.doWith(i)(processSurroundAndInclude("Source", xform(ns)))
        } yield nodes
      case se: scala.collection.Iterable[_] => runSourceContext(se.iterator, xform, ns)
      case se: scala.collection.Iterator[_] =>
        for {
          i <- se.toSeq; nodes <- currentSourceContext.doWith(i)(processSurroundAndInclude("Source", xform(ns)))
        } yield nodes
  ...
  private def findVisibleTemplate(path: ParsePath, session: Req): Box[NodeSeq] = {
    ...
    val splits = tpath.toList.filter { a =>
      !a.startsWith("_") && !a.startsWith(".") && a.toLowerCase.indexOf("-hidden") == -1
    } match {
  ...
  private def instantiateOrRedirect[T](c: Class[T]): Box[T] = {
    try {
      LiftSession.constructFrom(
        this,
        S.location.flatMap(_.currentValue.map(v => ParamPair(v, v.asInstanceOf[Object].getClass))),
        c
      )
  ...
  /** Split a string separated by a point or by a column in 2 parts. Uses default values if only one is found or if no
    * parts are found
    * @param in
    *   string to split
    * @return
  ...
  /** Report a snippet error depending on what the run mode is
    */
  private def reportSnippetError(
      page: String,
  ...
  /** During the HTTP request/response cycle or in a CometActor, Lift populates "S" with information about the current
    * session, the current request, etc. This method allows you to wrap a function in another function that will
    * snapshot current state (request vars, Req, Loc, etc.) such that when the returned function is executed, it will be
    * executed as if it had been executed in the scope of the thread where it was create. This allows you to farm work
    * out to separate threads, but make it look to those threads as if the scope was the same as if it had been executed
    * on the thread that created the function.
    */
  def buildDeferredFunction[T](deferredFunction: () => T): () => T = {
    ...
    () => {
      requestVarFunc(() => executeInScope(currentReq, renderVersion)(deferredFunction()))
    }
  }

  /** Overload of `buildDeferredFunction` for functions that take a parameter.
    *
    * The returned function, when invoked with a parameter of type `A`, will invoke the passed `deferredFunction` with
    * that parameter in the original request and session context.
    */
  def buildDeferredFunction[A, T](deferredFunction: (A) => T): (A) => T = {
    ...
    (in: A) => {
      requestVarFunc(() => executeInScope(currentReq, renderVersion)(deferredFunction(in)))
    }
  }
  ...
  private def processSnippet(
    ...
    def runWhitelist(snippet: String, cls: String, method: String, kids: NodeSeq)(f: => NodeSeq): NodeSeq = {
      ...
      if (pf.isDefinedAt(pair)) {
        val func = pf(pair)
        func.map(_.apply(kids)) openOr reportSnippetError(
          page,
          snippetName,
    ...
    val ret: NodeSeq =
      try {
        snippetName.map { snippet =>
          ...
          snippetTimer.timeSnippet(snippet) {
            S.doSnippet(snippet)(runWhitelist(snippet, cls, method, kids) {
              (S.locateMappedSnippet(snippet).map(_(kids)) or
                locSnippet(snippet)).openOr(S.locateSnippet(snippet).map(_(kids)) openOr {
                ...
                (locateAndCacheSnippet(cls)) match {
                  ...
                  case Full(inst) => {
                    ...
                    def nodeSeqFunc: Box[NodeSeq] =
                      ...
                      } yield meth.invoke(inst).asInstanceOf[Function1[NodeSeq, NodeSeq]].apply(kids)
                      ...
                      ((Helpers.invokeMethod(inst.getClass, inst, method, ar, Array(classOf[NodeSeq]))) or
                        ...
                        case it =>
                          val intersection = if (Props.devMode) {
                            ...
                            val methodAlts  = List(method, Helpers.camelify(method), Helpers.camelifyMethod(method))
          ...
          reportSnippetError(page, snippetName, LiftRules.SnippetFailures.NoNameSpecified, NodeSeq.Empty, wholeTag)
        }
      } catch {
        case ExclosedSnippetFailure(e) =>
          reportSnippetError(page, snippetName, e.snippetFailure, e.buildStackTrace, wholeTag)
    ...
    def checkAttr(attr_name: String, in: MetaData, base: MetaData): MetaData =
      in.filter(_.key == attr_name).toList match {
        case Nil => base
        case x   => new UnprefixedAttribute(attr_name, Text(x.head.value.text), base)
      }
      ...
      attrs.get("form").map(_.text.trim.toLowerCase) match {
        case Some("post") =>
          S.withAttrs(attrs.filter(_.key == "multipart")) {
            ...
            case e: Elem =>
              e % LiftRules.formAttrs.vend.foldLeft[MetaData](Null)((base, name) => checkAttr(name, attrs, base))
            case x => x
          }
        ...
        case Some(ft) =>
          <form action={S.uri} method={ft}>
            ...
            checkMultiPart(attrs) % LiftRules.formAttrs.vend.foldLeft[MetaData](Null)((base, name) =>
  ...
  /** Apply HTML specific corrections such as adding the context path etc.
    *
    * In general, you should heavily consider using `[[normalizeHtmlAndEventHandlers]]` or its friendliest sibling,
  ...
  /** Applies various HTML corrections to the passed HTML, including adding the context path to links and extracting
    * event handlers into a separate `JsCmd`.
    *
    * Note that most of the time you can just call `[[normalizeHtmlAndAppendEventHandlers]]` and not worry about the
    * extra `JsCmd`, as Lift will automatically append it to the response.
    *
    * @param forceExtractInlineJavaScript
    *   If `None`, uses `LiftRules.extractInlineJavaScript` to decide whether or not to extract inline JS from the
    *   passed nodes. If `Some`, extracts (`Some(true)`) or doesn't (`Some(false)`).
    */
  ...
  /** Runs `[[normalizeHtmlAndEventHandlers]]` to adjust URLs to context paths and extract JS event handlers from the
    * given `NodeSeq` and pull them into a separate JsCmd, then uses
    * `[[net.liftweb.http.S!.appendJs(js:net\.liftweb\.http\.js\.JsCmd):Unit* S.appendJs]]` to append that JS to the
    * response's JavaScript (the page-specific JS file if we are rendering a page, or the end of the returned JS if this
    * is an AJAX or comet request).
    */
  ...
  /** The partial function that defines how lift tags are processed for this session. Initially composed of
    * LiftRules.liftTagProcessing orElse the default lift tag processing. If you need to change the way a particular
  ...
  /** Pass in a LiftActor and get a JavaScript expression (function(x) {...}) that represents an asynchronous Actor
    ...
    *
    * In general, your Actor should be a subclass of ScopedLiftActor because that way you'll have the scope of the
    * current session.
    *
    * @param in
    ...
    *
    * @return
    *   a JsExp that contains a function that can be called with a parameter and when the function is called, the
    *   parameter is JSON serialized and sent to the server
    */
  ...
  /** Pass in a LiftActor and get a JavaScript expression (function(x) {...}) that represents an asynchronous Actor
    ...
    *   the Actor to send messages to.
    * @param xlate
    *   a function that will take the JsonAST.JValue and convert it into a representation that can be sent to the Actor
    *   (probably deserialize it into a case class.) If the translation succeeds, the translated message will be sent to
    *   the actor. If the translation fails, an error will be logged and the raw JsonAST.JValue will be sent to the
    *   actor
    *
    * @return
    *   a JsExp that contains a function that can be called with a parameter and when the function is called, the
            ...
            (p: JsonAST.JValue) => {
              in.!(xlate(p) match {
                ...
                case Failure(msg, Full(ex), _) =>
                  logger.error("Failed to deserialize JSON message " + p + ". Error " + msg, ex); p
  ...
  /** Create a Actor that will take messages on the server and then send them to the client. So, from the server
    * perspective, it's just an Async message send. From the client perspective, they get a function called each time
    * the message is sent from the server.
    *
    * If the message sent to the LiftActor is a JsCmd or JsExp, then the code is sent directly to the client and
    * executed on the client.
    *
    * If the message is a JsonAST.JValue, it's turned into a JSON string, sent to the client and the client calls the
    * function named in the `toCall` parameter with the value.
    *
    * If the message is anything else, we attempt to JSON serialize the message and if it can be JSON serialized, it's
    * sent over the wire and passed to the `toCall` function on the server
    *
    * @param toCall
    ...
    testStatefulFeature {
      val ca = new CometActor {
        ...
        /** It's the main method to override, to define what is rendered by the CometActor
          *
          * There are implicit conversions for a bunch of stuff to RenderOut (including NodeSeq). Thus, if you don't
          * declare the return turn to be something other than RenderOut and return something that's coercible into
          * RenderOut, the compiler "does the right thing"(tm) for you. <br/> There are implicit conversions for
          * NodeSeq, so you can return a pile of XML right here. There's an implicit conversion for NodeSeq => NodeSeq,
          * so you can return a function (e.g., a CssBindFunc) that will convert the defaultHtml to the correct output.
          * There's an implicit conversion from JsCmd, so you can return a pile of JavaScript that'll be shipped to the
          * browser.<br/> Note that the render method will be called each time a new browser tab is opened to the comet
          * component or the comet component is otherwise accessed during a full page load (this is true if a
          * partialUpdate has occurred.) You may want to look at the fixedRender method which is only called once and
          * sets up a stable rendering state.
          */
      ...
      ca.callInitCometActor(
        CometCreationInfo(Helpers.nextFuncName, Full(Helpers.nextFuncName), NodeSeq.Empty, Map.empty, this)
      )
        ...
        case elem @ DataAttrNode(toDo) =>
          toDo match {
            ...
            case DataAttributeProcessorAnswerFuture(nodeFuture) =>
              processOrDefer(true)(processSurroundAndInclude(page, nodeFuture.get(15000).openOr(NodeSeq.Empty)))
          }
          ...
          processOrDefer(isLazy) {
            S.withCurrentSnippetNodeSeq(elem) {
              S.doSnippet(snippetName) {
                S.withAttrs(attrs) {
                  processSurroundAndInclude(page, NamedPF((snippetName, element, attrs, kids, page), liftTagProcessing))
                }
              }
            }
          }
  ...
  def testStatefulFeature[T](f: => T): T = {
    if (this.stateful_?) f
    else throw new StateInStatelessException("Accessing stateful feature outside of a stateful session")
  }
  ...
  /** This method will send a message to a CometActor, whether or not the CometActor is instantiated. If the CometActor
    * already exists in the session, the message will be sent immediately. If the CometActor is not yet instantiated,
    * the message will be sent to the CometActor as part of setup `[[queueCometMessage]]` if it is created as part of
    * the current HTTP request/response cycle.
    *
    * @param theType
  ...
  /** Similar behavior to [[LiftSession#sendCometMessage(theType:String,msg:Any):Unit the main sendCometMessage]],
    * except that this version will limit based on the name of the comet. Providing `name` as `Empty`, will specifically
    * select comets with no name.
    *
    * @param theType
    *   the type of the CometActor
    * @param name
  ...
  /** Similar behavior to [[LiftSession#sendCometMessage(theType:String,msg:Any):Unit the main sendCometMessage]],
    * except that this version will limit based on the name of the comet and it takes its type selector as a type
    * parameter.
    *
    * @param name
    *   the optional name of the CometActor
  ...
  /** Find or build a comet actor of the given type `T` with the given configuration parameters. If a comet of that type
    * with that name already exists, it is returned; otherwise, a new one of that type is created and set up, then
    * returned.
    */
  ...
  /** As with `findOrBuildComet[T]`, but specify the type as a `String`. If the comet doesn't already exist, the comet
    * type is first looked up via `LiftRules.cometCreationFactory`, then `LiftRules.cometCreation`, and finally as a
    * class name in the comet packages designated by `LiftRules.buildPackage("comet")`.
    */
  ...
  /** Need to add your own CometActor to a Session/Page? You might 'cause creating a CometActor programmatically within
    * a snippet may be between that referring to a CometActor by class name.
    *
  ...
  private def buildCometByClass[T <: LiftCometActor](cometClass: Class[T])(creationInfo: CometCreationInfo): Box[T] = {
    ...
    def buildWithCreateInfoConstructor = {
      val constructor =
        cometClass.getConstructor(this.getClass, classOf[Box[String]], classOf[NodeSeq], classOf[Map[String, String]])
    ...
    attemptedComet match {
      case fail @ Failure(_, Full(e: java.lang.NoSuchMethodException), _) =>
        val message =
          s"Couldn't find valid comet constructor for ${cometClass.getCanonicalName}. Comets should have a no argument constructor or one that takes the following arguments: (LiftSession, Box[String], NodeSeq, Map[String,String])."
  ...
  /** Build a bunch of round-trip calls between the client and the server. The client calls the server with a parameter
    * and the parameter gets marshalled to the server and the code is executed on the server. The result can be an item
    * (JValue) or a Stream of Items.
    *
    * @param info
      ...
      val ca = new CometActor {
        ...
        override def lowPriority: PartialFunction[Any, Unit] = {
          ...
          case ItemMsg(guid, value) =>
            partialUpdate(
              JsCmds.JsSchedule(JsRaw(s"lift.sendEvent(${guid.encJs}, {'success': ${compactRender(value)}} )").cmd)
            )
          case DoneMsg(guid) =>
...
/** If a class is to be used as a lift view (rendering from code rather than a static template) and the method names are
  * to be used as "actions", the view must be marked as "InsecureLiftView" because there exists the ability to execute
  * arbitrary methods based on wire content
  */
trait InsecureLiftView

/** The preferred way to do lift views... implement a partial function that dispatches the incoming request to an
  * appropriate method
  */
trait LiftView {
    ...
    snippetInvocation.map { snip =>
      snip.charSplit('?') match {
        ...
        case snippetName :: snippetArguments =>
          SnippetInformation(
            urlDecode(removeLift(snippetName)),
            pairsToMetaData(snippetArguments.flatMap(_.roboSplit("[;&]")))
          )
      }
    }
  }
      ...
      next match {
        ...
        case up: UnprefixedAttribute if up.key == "class" =>
          up.value.text.charSplit(' ').filter(s => !isLiftClass(s)) match {
            case Nil =>
            case xs  => nonLift = new UnprefixedAttribute("class", xs.mkString(" "), nonLift)
          }

        case p: PrefixedAttribute if (p.pre == "l" || p.pre == "lift") && p.key == "parallel" => par = true

        case up: UnprefixedAttribute if up.key == "lift" || up.key == "data-lift" => // ignore
        case p: PrefixedAttribute if p.pre == "lift" && p.key == "snippet"        => nonLift = p.copy(nonLift)
    ...
    baseNode match {
      ...
      case elm: Elem => {
        for {
          ...
          val newElm         = new Elem(elm.prefix, elm.label, nonLift, elm.scope, elm.minimizeEmpty, elm.child: _*)
          (
            newElm,
            newElm,
            par ||
...
object RoundTripInfo {
  ...
  implicit def handledBuilder[T](in: (String, (T, RoundTripHandlerFunc) => Unit))(implicit
      m: Manifest[T]
  ): RoundTripInfo =
    HandledRoundTrip(in._1, in._2)(m)
}

/** A function (well, an interface with a bunch of methods on it) to call depending on the state of the round trip
  * function.
  */
trait RoundTripHandlerFunc {

  /** Send data back to the client. This may be called many times and each time, more data gets sent back to the client.
    * @param value
...
final case class StreamRoundTrip[T](name: String, func: T => Stream[Any])(implicit val manifest: Manifest[T])
    extends RoundTripInfo
final case class SimpleRoundTrip[T](name: String, func: T => Any)(implicit val manifest: Manifest[T])
    extends RoundTripInfo
final case class HandledRoundTrip[T](name: String, func: (T, RoundTripHandlerFunc) => Unit)(implicit
