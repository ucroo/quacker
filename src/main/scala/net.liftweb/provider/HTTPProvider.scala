...
/** Implement this trait in order to integrate Lift with other underlaying web containers. Not necessarily JEE
  * containers.
  */
trait HTTPProvider {
  ...
  protected def service(req: HTTPRequest, resp: HTTPResponse)(chain: => Unit) = {
    ...
    CurrentHTTPReqResp.doWith(req -> resp) {
      val newReq = Req(req, LiftRules.statelessRewrite.toList, Nil, LiftRules.statelessReqTest.toList, System.nanoTime)

      CurrentReq.doWith(newReq) {
        URLRewriter.doWith(url =>
          NamedPF.applyBox(resp.encodeUrl(url), LiftRules.urlDecorate.toList) openOr
            resp.encodeUrl(url)
        ) {
          if (
  ...
  protected def bootLift(loader: Box[String]): Unit = {
    try {
      LiftRules.setHTTPProvider(Some(this))
  ...
  private def postBoot {
    ...
    try {
      ...
      case _: Exception =>
        logger.error("LiftWeb core resource bundle for locale " + Locale.getDefault() + ", was not found ! ")
    } finally {
  ...
  /** Tests if a request should be handled by Lift or passed to the container to be executed by other potential filters
    * or servlets.
    */
